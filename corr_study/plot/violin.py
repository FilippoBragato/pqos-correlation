...
def multi_violinplot(distribution_data: [np.ndarray],
    ...
    if legend_num > 1:
        ...
        sns.violinplot(data=data, ax=ax, x=label_key, y=value_key, hue=legend_key, scale='width', split=legend_num>1, palette=palette, cut=0)

    else:

        data = pd.DataFrame({value_key: data_values, label_key: data_labels})

        sns.violinplot(data=data, ax=ax, x=label_key, y=value_key, scale='width', split=True, palette=palette, cut=0)
