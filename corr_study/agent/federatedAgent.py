...
class FederatedAgent(UEAgent):
    """Class representing an UE Agent in federated approach.
    ...
    def __init__(self, 
        ...
        self.primary_net.load_state_dict(starting_dict)
        target_net.load_state_dict(starting_dict)
...
class MultiFederatedAgent(Agent):
    def __init__(self, 
                 step_num: int, 
                 episode_num: int, 
                 state_dim: int, 
        ...
        self.users = {}
        self.users_imsi = []
        starting_net: Module = LinearNeuralNetwork(self.learning_state_dim, self.action_num)
        for imsi in range(1, user_num + 1):
            self.users[imsi] = FederatedAgent(imsi, 
                                                step_num, 
                                                episode_num, 
                                                state_dim, 
                                                action_num, 
        ...
        # keep a dictionary to compute the average for the update of the networks
        self.avg_params = {}

        # empty the dictionary for the next updates
        for key in starting_net.state_dict():
            self.avg_params[key] = torch.zeros(starting_net.state_dict()[key].size())
    ...
    def update_from_base(self):
        ...
        # empty the dictionary for the next updates
        for key in self.avg_params:
            self.avg_params[key] = torch.zeros(self.avg_params[key].size())

        for key in self.avg_params:
            for imsi in self.users_imsi:
                user = self.users[imsi]
                self.avg_params[key] = torch.add(self.avg_params[key], user.dql.primary_net.state_dict()[key])
            self.avg_params[key] = torch.mul(self.avg_params[key], 1/(self.user_num))

        # update the parameters of the networks for each user
        for imsi in self.users_imsi:
            self.users[imsi].dql.set_model(self.avg_params)
    ...
    def plot_data(self, data_folder: str, episode_num: int):
        ...
        state_palette = sns.color_palette('rocket_r', n_colors=self.learning_state_dim)
        action_palette = sns.color_palette('rocket_r', n_colors=3)
        user_palette = sns.color_palette('rocket_r', n_colors=self.user_num)
        single_palette = sns.color_palette('rocket_r', n_colors=1)
